...
//        Utils.printLog("SocketApp", "::GoingToPublishOn:" + channelName);
        isCurrentMsgPublished = false;
        tmp_channel.publish(message, (channelName1, error, data) -> {
            isCurrentMsgPublished = true;
//                Utils.printLog("SocketApp", "::PublishCallback:" + channelName);
            if (error == null) {
...
//        Utils.printLog("SocketApp", "111CalledFrom:ConfigSCConnection:" + message);
        if (!isDispatchThreadLocked) {
            isDispatchThreadLocked = true;

            Utils.printLog("SocketApp", "CalledFrom:ConfigSCConnection:" + message);
        ...
        exeWebServer.setDataResponseListener(responseString -> {
            if (responseString != null && Utils.checkText(responseString)) {
                ...
                if (isDataAvail) {
                    ...
                    if (currentDrivers != null && currentDrivers.length() > 0 && isClientKilled == false) {
